...
`<serial-port>` 示例：`/dev/cu.usbserial-110`。上传配置使用 PlatformIO 自带 `tool-stcgal`，协议 `stc8g`，下载波特率 `38400`，并通过 `custom_stcgal_trim = 11059` 把 IRC 设置到约 11.059MHz。如果个别芯片在 `Finishing write` 阶段偶发掉帧，可临时把 `custom_stcgal_baud` 降到 `9600` 复测。
