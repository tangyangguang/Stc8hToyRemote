...
| --- | --- | --- | --- | --- |
| controller | 7353/8192 | <= 7400 | 142 / >= 135 bytes | 5 / >= 0 bytes |
| receiver | 6617/8192 | <= 6800 | 158 / >= 145 bytes | 1 / >= 1 bytes |
...
6. 只有在 `Lxxx` 下双击 EC11 中键才进入扫描；两次按下间隔约 `10ms..0.5s` 都应能触发。
...
- 正常模式速度为 0 时 EC11 中键长按 5 秒进入配置模式。
- 速度不为 0 时按住 EC11 中键只清零速度，不进入配置模式；松开后在速度为 0 时重新长按 5 秒才进入配置模式。
- 配置模式显示 `P1/P2/P3/P4/P5` 和配置值。
- 配置模式短按切换配置项。
- 配置模式旋转只改选中的配置项。
- 舵机相关配置实时作用。
- 配置模式长按 5 秒保存并退出。
...
- P30/P31 为 P3.0/P3.1，低电平有效，固件启用内部上拉和数字输入；无外部上拉电阻时释放态也应读为高电平。
- 运行期单键按下并稳定约 30ms 后触发一次，在 32 个预设频道中切换并保存。默认顺序为 `76,36,40,44,48,52,56,60,64,68,72,80,83,32,34,38,42,46,50,54,58,62,66,70,74,78,82,35,43,51,59,67`。
- 按住单键不应连续高速切换；短抖动不应触发切换；双键同时按下不应切换频道，必须全部释放后才允许下一次单键切换。
...
- 手动扫描先快速扫一遍 32 个预设频道，再扫 `0..125` 全频道；每个频道连续发 2 个探测包，避免靠拉长单频道停留时间掩盖频道选择问题。
...
P30/P31 也是 STC8H UART1 默认下载相关引脚。烧录时不要按住 P30/P31，也不要让外部电路强拉低这两个脚；单纯外部上拉通常不影响烧录。
